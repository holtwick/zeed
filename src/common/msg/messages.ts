...
//           let message = queue[0]
//           try {
//             channel.postMessage(await promisify(encoder.encode(message)))
...
//         const { name, args, id } = await promisify(encoder.decode(msg.data))
//         if (name) {
//           log(`name ${name} id ${id}`)
//           try {
//             // @ts-ignore
...
//     channel.on("message", async (msg: any) => {
//       const { name, args, id, result, error } = await promisify(
//         encoder.decode(msg.data)
//       )
//       if (!name && id) {
